---
source_id: "SRC-robotics-420"
title: "MuJoCo model asset collision and inertia documentation"
...
aliases:
  - SRC-robotics-420
---
# MuJoCo model asset collision and inertia documentation
...
MuJoCo has a top-level stepper function [mj\_step](https://mujoco.readthedocs.io/en/stable/APIreference/APIfunctions.html#mj-step) which runs the entire forward dynamics and advances the state of the simulation. In many applications beyond simulation, however, it is beneficial to be able to run selected parts of the computation pipeline. To this end MuJoCo provides a large number of [flags](https://mujoco.readthedocs.io/en/stable/XMLreference.html#option-flag) which can be set in any combination, allowing the user to reconfigure the pipeline as needed, beyond the selection of algorithms and algorithm parameters via [options](https://mujoco.readthedocs.io/en/stable/XMLreference.html#option). Furthermore many lower-level functions can be called directly. User-defined callbacks can implement custom force fields, actuators, collision routines, and feedback controllers.

Model compilation

As mentioned above, the user defines a MuJoCo model in an XML file format called MJCF. This model is then compiled by the built-in compiler into the low-level data structure [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel), which is cross-indexed and optimized for runtime computation. The compiled model can also be saved in a binary MJB file.

Separation of model and data

MuJoCo separates simulation parameters into two data structures (C structs) at runtime:

- [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel) contains the model description and is expected to remain constant. There are other structures embedded in it that contain simulation and visualization options, and those options need to be changed occasionally, but this is done by the user.
- [mjData](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjdata) contains all dynamic variables and intermediate results. It is used as a scratch pad where all functions read their inputs and write their outputs – which then become the inputs to subsequent stages in the simulation pipeline. It also contains a preallocated and internally managed stack, so that the runtime module does not need to call memory allocation functions after the model is initialized.

[mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel) is constructed by the compiler. [mjData](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjdata) is constructed at runtime, given [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel). This separation makes it easy to simulate multiple models as well as multiple states and controls for each model, in turn facilitating [multi-threading](https://mujoco.readthedocs.io/en/stable/programming/simulation.html#simultithread) for sampling and [finite differences](https://mujoco.readthedocs.io/en/stable/APIreference/APIfunctions.html#mjd-transitionfd). The top-level API functions reflect this basic separation, and have the format:
...
The constraint solver automatically discovers independent [constraint islands](https://mujoco.readthedocs.io/en/stable/computation/index.html#soisland) – groups of bodies coupled by constraints that can be solved independently. This enables per-island parallelism and avoids over-solving: each island converges with the number of iterations it needs, and unconstrained degrees of freedom are skipped entirely. Additionally, [island sleeping](https://mujoco.readthedocs.io/en/stable/computation/index.html#sleeping) can freeze stationary islands, substantially reducing computation when parts of the scene are at rest.

Plugin system

MuJoCo supports [engine plugins](https://mujoco.readthedocs.io/en/stable/programming/extension.html) that extend the physics engine with custom elements: sensors, actuators, and passive forces. Plugins are dynamically loaded shared libraries that register themselves with the engine and are instantiated per model element. This allows extending MuJoCo without modifying the core engine.

GPU-accelerated backends

In addition to the C engine, MuJoCo has GPU-accelerated backends for large-scale parallel simulation: [MJX](https://mujoco.readthedocs.io/en/stable/mjx.html) (JAX) and [MuJoCo Warp](https://mujoco.readthedocs.io/en/stable/mjwarp/index.html) (NVIDIA Warp). Both backends consume the same `mjModel` and `mjData` structures, allowing seamless transfer between CPU and GPU. First-class [Python bindings](https://mujoco.readthedocs.io/en/stable/python.html) provide access to the full C API.
...
| **Memory** | [mjSpec](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjspec) (C struct) | [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel) (C struct) |

All runtime computations are performed with [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel) which is too complex to create manually. This is why we have two levels of modeling. The high level exists for user convenience: its sole purpose is to be compiled into a low level model on which computations can be performed. The resulting [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel) can be loaded and saved into a binary file (MJB), however those are version-specific and cannot be decompiled, thus models should always be maintained as XML files or packaged into MJZ archives.

The [mjSpec](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjspec) C struct is in one-to-one correspondence with the MJCF file format. The XML loader interprets the MJCF or URDF file, creates the corresponding [mjSpec](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjspec) and compiles it to [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel). The user can create [mjSpec](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjspec) programmatically and then save it to MJCF/MJZ or compile it. Procedural model creation and editing is described in the [Model Editing](https://mujoco.readthedocs.io/en/stable/programming/modeledit.html) chapter.

The following diagram shows the different paths to obtaining an [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel):
...
Models and specs can also be serialized or packaged back to disk using [mj\_encode](https://mujoco.readthedocs.io/en/stable/APIreference/APIfunctions.html#mj-encode).
...
[![_images/hello.png](https://mujoco.readthedocs.io/en/stable/_images/hello.png)](https://mujoco.readthedocs.io/en/stable/_images/hello.png)
...
<video controls="" width="200" height="295"><source src="https://mujoco.readthedocs.io/en/stable/_static/example.mp4" type="video/mp4"></video>
...
Divergence of a simulation happens when elements of the state tend quickly to infinity. In MuJoCo this is usually manifested as an [mjWARN\_BADQACC](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjtwarning) warning. Divergence is endemic to all physics simulation and is not necessarily indicative of a bad model or bug in the simulator, but is rather a hint that the timestep is too large for the given choice of integrator. In physics simulation there is always a tension between speed (large time steps) and stability (small timesteps). A model which is well-tuned for speed has the largest possible timestep that does not diverge, which usually means that it *can* be made to diverge under extreme conditions. In that sense *rare* cases of divergence can actually be indicative of a well-tuned model. In all cases it should be possible to prevent divergence by reducing the timestep and/or switching to a more stable [integrator](https://mujoco.readthedocs.io/en/stable/computation/index.html#geintegration). If that fails, the culprit is different. For example in models where bodies are initialized in penetration, large repulsive forces could push them away and cause divergence.
...
- The default value of [gravity](https://mujoco.readthedocs.io/en/stable/XMLreference.html#option) is (0, 0, -9.81), which corresponds to Earth surface gravity in MKS. Note that this does not really specify the MKS system of units, since we might be using CGS on [Enceladus](https://en.wikipedia.org/wiki/Enceladus).
- The default value of [geom density](https://mujoco.readthedocs.io/en/stable/XMLreference.html#body-geom) (used to infer body masses and inertias) is 1000, which corresponds to the density of water in MKS.
...
**Angles:** Although angles can be specified using degrees in MJCF (and indeed degrees are the [default](https://mujoco.readthedocs.io/en/stable/XMLreference.html#compiler)), all angular quantities [mjModel](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjmodel) and [mjData](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjdata) are expressed in [radians](https://en.wikipedia.org/wiki/Radian). So, for example, if we are using MKS, angular velocities reported by [gyroscopes](https://mujoco.readthedocs.io/en/stable/XMLreference.html#sensor-gyro) would be in rad/s while stiffness of hinge joints would be in Nm/rad.

### Surprising Collisions

MuJoCo by default excludes collisions between geoms that belong to body pairs which have a direct parent-child relationship. For example, consider the arm model in the section above: there is no collision at the “elbow” even though the capsule geoms are penetrating, because the forearm is an immediate child of the upper arm.

However, this exclusion is **not applied if the parent is a static body** i.e., the world body, or a body without any degrees of freedom relative to the world body. This behavior, documented in the [Collision detection](https://mujoco.readthedocs.io/en/stable/computation/index.html#collision) section, prevents objects from falling through the floor or moving through walls. However, this behavior often leads to the following situation:
...
1. Don’t remove the root joint. Perhaps it is enough to [disable gravity](https://mujoco.readthedocs.io/en/stable/XMLreference.html#option-flag) and possibly add some [fluid viscosity](https://mujoco.readthedocs.io/en/stable/XMLreference.html#option) in order to prevent your model from moving around too much.
2. Use [collision filtering](https://mujoco.readthedocs.io/en/stable/computation/index.html#collision) to explicitly disable the unwanted collisions, either by setting the relevant contype and conaffinity attributes, or by using a contact [exclude](https://mujoco.readthedocs.io/en/stable/XMLreference.html#contact-exclude) directive.
...
For situations where it is desirable to suppress slip completely, there is a [NoSlip](https://mujoco.readthedocs.io/en/stable/computation/index.html#soalgorithms) post-processing solver which runs after the main solver. It updates the contact forces in friction dimensions by disregarding constraint softness. When this option is used however, MuJoCo is no longer solving the convex optimization problem it was designed to solve, and the simulation may become less robust. Thus using the Newton solver with elliptic friction cones and large value of `impratio` is the recommended way of reducing slip. For more detailed recommendations, see [preventing slip](https://mujoco.readthedocs.io/en/stable/modeling.html#cslippage) in the Modeling chapter.
...
The available element types are defined in [mjtype.h](https://github.com/google-deepmind/mujoco/blob/main/include/mujoco/mjtype.h), in the enum type [mjtObj](https://mujoco.readthedocs.io/en/stable/APIreference/APItypes.html#mjtobj). These enums are mostly used internally. One exception are the functions [mj\_name2id](https://mujoco.readthedocs.io/en/stable/APIreference/APIfunctions.html#mj-name2id) and [mj\_id2name](https://mujoco.readthedocs.io/en/stable/APIreference/APIfunctions.html#mj-id2name) in the MuJoCo API, which map element names to integer ids and vice versa. These functions take an element type as input.
...
First the similarities. Bodies, geoms and sites all have spatial frames attached to them (although bodies also have a second frame which is centered at the body center of mass and aligned with the principal axes of inertia). The positions and orientations of these frames are computed at each time step from `mjData.qpos` via forward kinematics. The results of forward kinematics are available in `mjData` as xpos, xquat and xmat for bodies, geom\_xpos and geom\_xmat for geoms, site\_xpos and site\_xmat for sites.
...
![_images/bodygeomsite.png](https://mujoco.readthedocs.io/en/stable/_images/bodygeomsite.png)
...
Free joints are always defined in the body frame, yet it is computationally favorable to align this frame with the body’s inertia. Read more about this option in the documentation of the [freejoint/align](https://mujoco.readthedocs.io/en/stable/XMLreference.html#body-freejoint-align) attribute.
