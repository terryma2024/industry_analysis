...
- For every deep-research task, follow `.agents/skills/industry-analysis/references/research-report-taxonomy.md`: classify it as one primary type (`R01`–`R10`) with up to two secondary types before expanding the research. For article, Xiaohongshu, Bilibili, podcast, interview, or other content-led research, extract source claims first, then classify and apply the matching fixed report requirements. If no type fits, use the `R00` chain: industry panorama → key chain segment → technology and companies → products and scenarios → commercial reality → cost and market space → investment, entrepreneurship, or career decision.
- Every deep-research report, regardless of classification, must include explicit `商业应用可能性` and `中小型创业者的机会` sections. Do not omit them when evidence is weak; state `待验证`, explain why, and identify the next validation step.
...
- Maintain the global LLM Wiki files:
  ...
  - `knowledge/log.md` is append-only and grouped by date. Keep exactly one `## [YYYY-MM-DD]` section per day; append each same-day event as `- **action | summary**` with indented details for changed pages, sources, conclusions, conflicts, and limitations. Do not create duplicate date headings.
