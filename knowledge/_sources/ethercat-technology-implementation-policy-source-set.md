...
sources:
  ...
  - raw/robotics-embodied-ai/documents/SRC-robotics-508-ethercat-implementation-guide.md
...
> [!summary]
> 本来源集以 ETG 官方技术、标准化、许可、实现、EtherCAT G/TSN 文档为主，以 SOEM、IgH 官方代码资料和 ODVA CIP Motion 官方页作实现与竞品对照，并用 IETF TCP/UDP 标准限定“EtherCAT 比 TCP/IP 快”的语义、用工信部文件限定中国政策表述。稳定结论是：EtherCAT 是开放标准化、受专利和兼容性许可治理的实时工业以太网技术；它并不等于“协议开源”，也不能仅凭总线性能推导整机控制实时性。
...
| [`SRC-robotics-508`](../../raw/robotics-embodied-ai/documents/SRC-robotics-508-ethercat-implementation-guide.md) | 从站实现、状态机、ESI、同步与测试 | S | 官方 Implementation Guide；采用版本需冻结 |
...
| [`SRC-robotics-530`](../../raw/robotics-embodied-ai/documents/SRC-robotics-530-rfc-9293-transmission-control-protocol-tcp.md) | TCP 可靠、有序、ACK、重传和拥塞控制语义 | S | 定义传输协议，不代表某个优化机器人实现的实测性能 |
| [`SRC-robotics-531`](../../raw/robotics-embodied-ai/documents/SRC-robotics-531-rfc-768-user-datagram-protocol.md) | UDP 最小 datagram 机制及无送达/有序保证 | S | 只定义 UDP；实时同步和设备模型仍由应用补齐 |
...
- TCP/IP 峰值吞吐量不必低于 EtherCAT；“EtherCAT 更快”仅指受控多轴周期交换的时延、尾时延、抖动与同步优势，需用统一台架验证。

## 关联连接

- [[robotics-embodied-ai/research-notes/ethercat-technology-ecosystem-engineering-deep-dive-2026-08-09|EtherCAT 深度调研]]
- [[robotics-embodied-ai/research-notes/ethercat-vs-tcp-ip-robot-control-latency-2026-08-12|EtherCAT vs TCP/IP 机器人控制时延专题]]
- [[embodied-ai|具身智能]]
