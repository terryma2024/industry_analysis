...
> [!summary]
> UMI Gripper 的价值不只是一个低成本夹爪，而是一套“物理接口 + 数据处理 + 策略接口”的标准化方案：用人手拿着类机器人夹爪在真实环境示教，再把视频、IMU、末端位姿、夹爪宽度等信息转换成可训练的机器人轨迹。对国内创业或业务进入点而言，机会不在复刻开源夹爪本身，而在做成可交付的设备、数据质量体系、操作 SOP、格式转换、基线训练和垂直场景数据服务。
...
| GoPro + 鱼眼 | 155 度广角，直接使用 raw fisheye 图像 | 腕部视角有限，广角能补场景上下文 | 可评估 GoPro、Insta360、工业鱼眼相机、国产运动相机，但要保证 IMU、时间戳和画质。 |
| 侧镜 | 在同一张图里形成隐式多视角/类立体信息 | 补深度线索，减少单目视觉局限 | FastUMI 去掉侧镜以留出传感器空间；是否保留要按任务实测。 |
...
| 机器人端同构视角 | 机器人夹爪也安装相同视角相机和指尖 | 减少人类示教和机器人部署的 observation gap | ToB 交付必须为不同 gripper 做适配件和视觉对齐夹具。 |
...
详表见 [umi_related_implementations.csv](../../raw/robotics-embodied-ai/data/umi_related_implementations.csv)。

| 路线 | 代表系统 | 解决了什么 | 新问题 |
|---|---|---|---|
| 原始 UMI | UMI | 低成本、便携、跨机器人、真实环境示教 | SLAM 对弱纹理/遮挡/动态场景敏感，复现门槛不低。 |
| 工程简化 | FastUMI | 用 T265 直接获得 6DoF pose，降低 SLAM 部署复杂度，适配多种 gripper | T265 供应和长期可得性风险，商业产品不能押单一停产传感器。 |
| 3D 感知增强 | UMI-3D | 用低成本 LiDAR 和 3D SLAM 应对白墙、遮挡、动态物体、门/窗帘等任务 | 硬件、标定和数据处理复杂度上升。 |
| 力/触觉增强 | UMI-FT、TacUMI | 解决擦拭、插入、线缆、接触丰富任务中的力调制和事件切分 | 传感器成本、耐用性、漂移、标定和维修会成为 ToB 难点。 |
...
| 家庭低成本工具 | Dobb-E | 用 reacher-grabber + iPhone 低成本采家庭数据 | 动作空间和机器人 embodiment gap 更大。 |
...
- 输出一张你自己的数据流图：人拿夹爪示教 -> MP4/IMU/width -> SLAM -> Zarr -> DP/ACT -> robot rollout。
...
- 数据工程：episode、采样率、同步、Zarr、ReplayBuffer、视频压缩。
...
- 打开 Zarr 看字段：`camera0_rgb`、`robot0_eef_pos`、`robot0_eef_rot_axis_angle`、`robot0_gripper_width`、`episode_ends`。
...
- 硬件：被动 UMI-like 夹爪一版，尽量使用国内可采购零件；如果走 FastUMI 路线，同时准备 T265 替代方案。
...
- 硬件 DFM：轻量化、耐用、快拆、模块化传感器、标准安装接口。
...
| 供应链风险 | GoPro、T265、力触觉传感器、LiDAR 都可能有供应或成本问题 | 国产替代清单和长期供货协议。 |
...
- 把 `umi_related_implementations.csv` 增加字段：BOM、可采购链接、许可证、是否仍可买到关键传感器、国内替代件。
- 选一个 v0 任务，建立 100 条演示采集 SOP 和质检表。
- 做 UMI/Zarr 与 LeRobot schema 对照表。
- 对 IO-AI、Robotin、FirstMove、国内机器人整机厂的数据服务能力做二次交叉验证。
- 做一页“给潜在客户看的数据包样例目录”：原始、处理后、标注、质量报告、baseline 结果、失败样例。
