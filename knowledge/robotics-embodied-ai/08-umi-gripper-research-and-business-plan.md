...
> [!summary]
> [[_entities/UniversalManipulationInterface|UMI]] [[_entities/Gripper|Gripper]] 的价值不只是一个低成本夹爪，而是一套“物理接口 + 数据处理 + 策略接口”的标准化方案：用人手拿着类机器人夹爪在真实环境示教，再把视频、[[_entities/IMU|IMU]]、[[_entities/EndEffector|末端位姿]]、[[_entities/GripperWidth|夹爪宽度]]等信息转换成可训练的机器人[[_entities/Trajectory|轨迹]]。对国内创业或业务进入点而言，机会不在复刻开源夹爪本身，而在做成可交付的设备、数据质量体系、操作 [[_entities/StandardOperatingProcedure|SOP]]、格式转换、[[_entities/Baseline|基线训练]]和垂直场景数据服务。

> [!tip] 术语辅助阅读
> 初学者遇到报告里的技术词，可以先看 [[_entities/README|UMI Gripper 初学者技术术语教学]]。常见入口：[[_entities/IMU|IMU]]、[[_entities/SixDoF|6DoF]]、[[_entities/SLAM|SLAM]]、[[_entities/Zarr|Zarr]]、[[_entities/HuggingFaceLeRobot|LeRobot]]、[[_entities/DiffusionPolicy|Diffusion Policy]]、[[_entities/ActionChunkingTransformer|ACT]]、[[_entities/Episode|episode]]、[[_entities/TimeSynchronization|时间同步]]。
...
- **UMI 试图解决的问题**：机器人遥操作数据质量高但贵、慢、依赖真实机器人；纯人类视频便宜但缺动作和机器人可执行性。UMI 站在中间：让人拿着“机器人化的[[_entities/Gripper|手持夹爪]]”做任务，尽量同时保留人类示教的自然性和机器人动作空间的可迁移性。证据：[`SRC-robotics-066`](../../raw/robotics-embodied-ai/documents/SRC-robotics-066-universal-manipulation-interface-in-the-wild-robot-teaching-without-in-the-wild.md)
- **核心设计**：手持 [[_entities/ParallelGripper|3D 打印平行夹爪]] + [[_entities/GoPro|GoPro]] [[_entities/FisheyeCamera|鱼眼相机]] + 侧镜 + GoPro [[_entities/IMU|IMU]] + [[_entities/Marker|marker]] 夹爪宽度追踪 + [[_entities/VisualInertialSLAM|视觉惯性 SLAM]] + [[_entities/RelativePose|相对末端轨迹]]动作表示 + 推理时[[_entities/Latency|延迟]]匹配 + [[_entities/DiffusionPolicy|Diffusion Policy]]。证据：[`SRC-robotics-065`](../../raw/robotics-embodied-ai/documents/SRC-robotics-065-universal-manipulation-interface-project-page.md) [`SRC-robotics-066`](../../raw/robotics-embodied-ai/documents/SRC-robotics-066-universal-manipulation-interface-in-the-wild-robot-teaching-without-in-the-wild.md)
- **商业化直觉**：卖“夹爪套件”只是入口；真正 [[_entities/ToB|ToB]] 价值是帮助客户稳定获得可训练、可复现、可评估、能导出到 UMI/[[_entities/Zarr|Zarr]]/[[_entities/HuggingFaceLeRobot|LeRobot]]/[[_entities/ActionChunkingTransformer|ACT]]/[[_entities/DiffusionPolicy|DP]] 的[[_entities/DataPackage|数据包]]。
...
| GoPro + 鱼眼 | 155 度广角，直接使用 raw [[_entities/FisheyeCamera\|fisheye]] 图像 | [[_entities/WristView\|腕部视角]]有限，广角能补场景上下文 | 可评估 GoPro、Insta360、工业鱼眼相机、国产运动相机，但要保证 IMU、时间戳和画质。 |
| 侧镜 | 在同一张图里形成隐式[[_entities/MultiView\|多视角]]/类立体信息 | 补深度线索，减少单目视觉局限 | FastUMI 去掉侧镜以留出传感器空间；是否保留要按任务实测。 |
...
| 机器人端同构视角 | 机器人夹爪也安装相同视角相机和指尖 | 减少人类示教和机器人部署的 [[_entities/ObservationGap\|observation gap]] | ToB 交付必须为不同 gripper 做适配件和视觉对齐夹具。 |

### 数据与策略接口

| 设计 | 解释 | 业务含义 |
|---|---|---|
| 视觉惯性 SLAM | UMI 用 [[_entities/ORBSLAM3\|ORB-SLAM3]] 分支从 GoPro 视频和 IMU 恢复带尺度的 [[_entities/SixDoF\|6DoF]] 轨迹。官方 README 也提醒 SLAM 是较脆弱环节。证据：[`SRC-robotics-067`](../../raw/robotics-embodied-ai/documents/SRC-robotics-067-universal-manipulation-interface-github-repository.md) | 国内产品要把“SLAM 成功率、失败原因、轨迹质量报告”做成核心卖点。 |
| 相对末端轨迹 | 动作用一段相对当前 [[_entities/EndEffector\|EE]] [[_entities/Pose\|pose]] 的 [[_entities/SE3\|SE(3)]] 轨迹表示，避免依赖全局[[_entities/CoordinateFrame\|坐标系]]。证据：[`SRC-robotics-066`](../../raw/robotics-embodied-ai/documents/SRC-robotics-066-universal-manipulation-interface-in-the-wild-robot-teaching-without-in-the-wild.md) | 数据跨机器人可迁移的关键。客户如果只要某一台机器人，GELLO/ALOHA 式直接遥操作可能更简单。 |
| 推理时延迟匹配 | UMI 在推理时对相机、机器人、夹爪的 [[_entities/Observation\|observation]]/[[_entities/Action\|action]] latency 做对齐和提前发指令。证据：[`SRC-robotics-066`](../../raw/robotics-embodied-ai/documents/SRC-robotics-066-universal-manipulation-interface-in-the-wild-robot-teaching-without-in-the-wild.md) | 服务交付必须包含延迟测量，不然动态任务会出现抖动、错位和失败。 |
| Zarr 数据格式 | UMI 社区把 GoPro、SLAM 输出和 Zarr 作为数据层级；Zarr 里包含 camera [[_entities/RGB\|RGB]]、demo start/end pose、eef pos/rot、gripper width 和 episode_ends。证据：[`SRC-robotics-068`](../../raw/robotics-embodied-ai/documents/SRC-robotics-068-umi-robot-dataset-community.md) | 商业数据包应支持 Zarr 和 LeRobot 双导出，附 [[_entities/DatasetSchema\|schema]]、版本、[[_entities/SamplingRate\|采样率]]、[[_entities/Compression\|压缩方式]]和[[_entities/QualityControl\|质检报告]]。 |
| 模仿学习算法 | UMI 原文主要使用 Diffusion Policy，同时指出 ACT 可作为替代。证据：[`SRC-robotics-066`](../../raw/robotics-embodied-ai/documents/SRC-robotics-066-universal-manipulation-interface-in-the-wild-robot-teaching-without-in-the-wild.md) | 服务不应绑定单一算法，应交付 [[_entities/BaselineTrainingRecipe\|baseline training recipe]] 和可复现配置。 |
...
| 工程简化 | FastUMI | 用 [[_entities/IntelRealSenseT265\|T265]] 直接获得 6DoF pose，降低 SLAM 部署复杂度，适配多种 gripper | [[_entities/IntelRealSenseT265\|T265]] 供应和长期可得性风险，商业产品不能押单一停产传感器。 |
| 3D 感知增强 | UMI-3D | 用低成本 [[_entities/LiDAR\|LiDAR]] 和 [[_entities/ThreeDSLAM\|3D SLAM]] 应对白墙、遮挡、动态物体、门/窗帘等任务 | 硬件、[[_entities/Calibration\|标定]]和数据处理复杂度上升。 |
| 力/触觉增强 | UMI-FT、TacUMI | 解决擦拭、插入、线缆、接触丰富任务中的力调制和事件切分 | [[_entities/ForceTorqueSensor\|力/力矩传感器]]、[[_entities/TactileSensor\|触觉传感器]]成本、耐用性、漂移、标定和维修会成为 ToB 难点。 |
...
| 家庭低成本工具 | Dobb-E | 用 reacher-grabber + iPhone 低成本采家庭数据 | 动作空间和机器人 [[_entities/EmbodimentGap\|embodiment gap]] 更大。 |
...
- 输出一张你自己的数据流图：人拿夹爪示教 -> [[_entities/MP4|MP4]]/IMU/width -> SLAM -> Zarr -> DP/ACT -> robot [[_entities/Rollout|rollout]]。
...
- 数据工程：[[_entities/Episode|episode]]、采样率、同步、Zarr、[[_entities/ReplayBuffer|ReplayBuffer]]、视频压缩。
...
- 打开 Zarr 看字段：`camera0_rgb`、`robot0_eef_pos`、`robot0_eef_rot_axis_angle`、`robot0_gripper_width`、`episode_ends`；其中 `rot_axis_angle` 可先理解为[[_entities/AxisAngle|轴角]]旋转表示。
...
- 硬件：被动 UMI-like 夹爪一版，尽量使用国内可采购零件；如果走 FastUMI 路线，同时准备 [[_entities/IntelRealSenseT265|T265]] 替代方案。
...
- 硬件 [[_entities/DesignForManufacturing|DFM]]：轻量化、耐用、快拆、模块化传感器、标准安装接口。
...
| 供应链风险 | GoPro、[[_entities/IntelRealSenseT265\|T265]]、力触觉传感器、LiDAR 都可能有供应或成本问题 | 国产替代清单和长期供货协议。 |
...
| 任务 | 状态 | 产出/链接 | 仍待验证 |
|---|---|---|---|
| 把 `umi_related_implementations.csv` 增加字段：[[_entities/BillOfMaterials\|BOM]]、可采购链接、许可证、是否仍可买到关键传感器、国内替代件 | 已补到原表 | [umi_related_implementations.csv](../../raw/robotics-embodied-ai/data/umi_related_implementations.csv)、[umi_hardware_bom_and_localization.csv](../../raw/robotics-embodied-ai/data/umi_hardware_bom_and_localization.csv)、[[research-notes/umi-hardware-localization-2026-05-27]] | Dobb-E、Data Scaling Laws with UMI 未做硬件 BOM 核验；部分 Google Doc/采购页仍需人工复核。 |
